...
Status: **active; v0.1.2 released with green multi-OS CI and private source consumption; production consumer and final authority closure open**
...
| ID | Oracle | Status |
| --- | --- | --- |
| ENG01 | Independent public package has license/docs/build/test/typed API | PASS — public repository and immutable v0.1.2 tag at commit `98a8b08`; release verification recorded in the E3 contract |
...
| ENG08 | Private service consumes released engine and duplicates are deleted | PARTIAL — private source admission/worker/package migration consumes the hash-pinned v0.1.2 release; production image proof and duplicate-authority closure remain open |
...
| ENG10 | macOS/Windows/Linux clean install and uninstall-preserves-data | PARTIAL — both exact-v0.1.2 workflows are green across macOS/Ubuntu/Windows and Python 3.10/3.12; independent install/uninstall-preserves-data evidence remains open |
| ENG11 | Local/cloud scientific equivalence under declared tolerance | PARTIAL — one deterministic Brian fixture is exactly equal after normalizing only attempt `run_id`; broader backend/artifact coverage remains open |
| ENG12 | Public release/tag/package/docs/security evidence | PARTIAL — v0.1.2 source/tag/archive checksum/docs and twelve-job CI evidence exist; signing and final security/reproducibility closure remain open |
